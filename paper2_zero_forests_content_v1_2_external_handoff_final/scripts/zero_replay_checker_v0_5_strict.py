...
def recompute_subleaf_hash(leaf: Dict[str, Any]) -> str:
    tmp = dict(leaf)
    tmp.pop("subleaf_hash", None)
    return hash16(tmp)
...
def leaf_payload(leaves: Dict[str, Dict[str, Any]], sid: str) -> Dict[str, Any]:
    return leaves[sid].get("payload", {})

def validate_flrw_subleaves(cert: Dict[str, Any], issues: List[Dict[str, Any]]) -> Dict[str, Any]:
    claim = cert.get("claim_id")
    leaves_list = cert.get("tensor_subleaves", [])
    leaves = {x.get("subleaf_id"): x for x in leaves_list}
    checks = []
    def chk(name: str, ok: bool, diagnostic: str = ""):
        checks.append({"check": name, "ok": bool(ok), "diagnostic": diagnostic})
        if not ok:
            add_issue(issues, "RC9", "FLRW_SUBLEAF_CHECK_FAILED", claim_id=claim, check=name, diagnostic=diagnostic)
    missing = sorted(FLRW_REQUIRED_SUBLEAVES - set(leaves))
    chk("required_subleaves_present", not missing, "missing=" + repr(missing))
    for sid, leaf in leaves.items():
        reported = leaf.get("subleaf_hash")
        recomputed = recompute_subleaf_hash(leaf)
        ok = (reported == recomputed)
        chk(f"subleaf_hash::{sid}", ok, f"reported={reported}, recomputed={recomputed}")
        for dep in leaf.get("dependencies", []):
            chk(f"dependency_exists::{sid}->{dep}", dep in leaves, "")
    if missing:
        return {"subleaf_count": len(leaves_list), "checks": checks}
    div_deps = set(leaves["flrw.divergence.expanded_nu0"].get("dependencies", []))
    chk("divergence_required_dependencies", div_deps == FLRW_REQUIRED_DIVERGENCE_DEPS, f"deps={sorted(div_deps)}")
    # Scalar-selected checks.
    p_metric = leaf_payload(leaves, "flrw.metric.scale_factor")
    a = p_metric.get("a_expr", "NaN")
    gii = p_metric.get("g_ii", "NaN")
    gii_inv = p_metric.get("g_contra_ii", "NaN")
    ok, diag = expr_equal(gii, f"({a})**2")
    chk("metric_gii_equals_a_squared", ok, diag)
    ok, diag = expr_equal(f"({gii})*({gii_inv})", "1")
    chk("metric_inverse_spatial_diagonal", ok, diag)
    p_trace = leaf_payload(leaves, "flrw.connection.gamma_trace_0")
    adot_over_a = p_trace.get("a_dot_over_a_expr", "NaN")
    gamma_trace = p_trace.get("gamma_trace_0_expr", "NaN")
    ok, diag = expr_equal(adot_over_a, "H")
    chk("a_dot_over_a_equals_H", ok, diag)
    ok, diag = expr_equal(gamma_trace, "3*H")
    chk("gamma_trace_0_equals_3H", ok, diag)
    p_g0ii = leaf_payload(leaves, "flrw.connection.gamma_0ii")
    gamma_0ii = p_g0ii.get("gamma_0ii_expr", "NaN")
    ok, diag = expr_equal(gamma_0ii, "H*exp(2*H*t)")
    chk("gamma_0ii_selected_component", ok, diag)
    p_G00 = leaf_payload(leaves, "flrw.einstein.G_contra_00")
    G00 = p_G00.get("G_contra_00_expr", "NaN")
    ok, diag = expr_equal(G00, "3*H**2")
    chk("G_contra_00_selected_component", ok, diag)
    p_Gii = leaf_payload(leaves, "flrw.einstein.G_contra_ii")
    Gii = p_Gii.get("G_contra_ii_expr", "NaN")
    ok, diag = expr_equal(Gii, "-3*H**2*exp(-2*H*t)")
    chk("G_contra_ii_selected_component", ok, diag)
    p_div = leaf_payload(leaves, "flrw.divergence.expanded_nu0")
    partial = p_div.get("partial_G00_expr", "NaN")
    trace_term = p_div.get("gamma_trace_term_expr", "NaN")
    spatial = p_div.get("spatial_connection_term_expr", "NaN")
    div_expr = p_div.get("divergence_expanded_expr", "NaN")
    expected_from_subleaves = f"({partial}) + ({gamma_trace})*({G00}) + 3*({gamma_0ii})*({Gii})"
    ok, diag = expr_equal(div_expr, expected_from_subleaves)
    chk("divergence_expr_matches_named_subleaves", ok, diag)
    ok, diag = expr_equal(trace_term, f"({gamma_trace})*({G00})")
    chk("gamma_trace_term_matches_subleaves", ok, diag)
    ok, diag = expr_equal(spatial, f"3*({gamma_0ii})*({Gii})")
    chk("spatial_connection_term_matches_subleaves", ok, diag)
    ok, diag = expr_equal(div_expr, cert.get("normalized_obligation", {}).get("lhs_expr", "NaN"))
    chk("divergence_expr_matches_normalized_lhs", ok, diag)
    ok, diag = expr_zero(div_expr)
    chk("divergence_expr_simplifies_to_zero", ok, diag)
    # Route tensor steps must point to named subleaves.
    for route in cert.get("routes", []):
        tensor_ref_union: Set[str] = set()
        for st in route.get("steps", []):
            if st.get("rule_class") in TENSOR_CLASSES:
                tensor_ref_union |= set(st.get("subleaf_refs", []))
        chk(f"route_subleaf_refs::{route.get('route_id')}", "flrw.divergence.expanded_nu0" in tensor_ref_union and FLRW_REQUIRED_DIVERGENCE_DEPS.issubset(tensor_ref_union), f"refs={sorted(tensor_ref_union)}")
    return {"subleaf_count": len(leaves_list), "checks": checks}
...
def replay_bundle(bundle: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for cert in bundle.get("certificates", []):
        ...
        if claim == "zero.flrw.contracted_bianchi_divG0":
            tensor_subleaf_reports.append({"claim_id": claim, **validate_flrw_subleaves(cert, issues)})
    ...
    gates = {g: True for g in ["SCHEMA","RC1","RC2","RC3","RC4","RC5","RC6","RC7","RC8","RC9"]}
    for i in issues:
        gates[i.get("gate")] = False
    err_count = len(issues)
    return {
        "title":"Paper2 Zero Forests strict Level-B+ replay report v0.5 tensor subleaves",
        "review_pass": err_count == 0,
        "strict_level":"Level-B+ named tensor subleaf seed content",
        "schema_version": bundle.get("schema_version"),
        "artifact_version": bundle.get("artifact_version"),
        "content_extension": bundle.get("content_extension"),
        "counts": {"zero_claims":len(bundle.get("certificates", [])), "routes":route_count, "steps":step_count, "routes_ok":route_ok, "steps_ok":step_ok, "raw_hashes_recomputed_ok":raw_ok, "normalized_hashes_recomputed_ok":norm_ok, "unique_normalized_certificates":len({x.get("normalized_hash") for x in bundle.get("certificates", [])}), "tensor_subleaves": sum(len(x.get("tensor_subleaves", [])) for x in bundle.get("certificates", []))},
        "gates": gates,
        ...
        "tensor_subleaf_reports": tensor_subleaf_reports,
    }
